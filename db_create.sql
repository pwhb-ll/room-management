...
CREATE TABLE IF NOT EXISTS public.users (
  ...
  role roles NOT NULL DEFAULT 'guest'::roles,
...
BEGIN
  ...
  IF NOT FOUND THEN
    INSERT INTO users(role, password, email, first_name, last_name, phone)
      VALUES('guest', crypt(input_password, gen_salt('bf', 8)), input_email, input_first_name, input_last_name, input_phone)
      RETURNING
        json_build_object(
          'sessionId', create_session(users.id),
          'user', json_build_object('id', users.id, 'role', 'guest', 'email', input_email, 'firstName', input_first_name, 'lastName', input_last_name, 'phone', input_phone, 'optOut', false)
        ) INTO user_session;
...
BEGIN
  ...
  IF NOT FOUND THEN
    INSERT INTO users(role, email, first_name, last_name)
      VALUES('guest', input_email, input_first_name, input_last_name)
      RETURNING
        json_build_object(
          'id', create_session(users.id),
          'user', json_build_object('id', users.id, 'role', 'guest', 'email', input_email, 'firstName', input_first_name, 'lastName', input_last_name, 'phone', null)
        ) INTO user_session;
  END IF;
END;
$BODY$;
...
DECLARE
  ...
  input_role roles := COALESCE((input->>'role')::roles, 'guest');
  input_email varchar(80) := LOWER(TRIM((input->>'email')::varchar));
...
CALL public.upsert_user('{"id":0, "role":"admin", "email":"llops@linuxlab.pro", "password":"Google2022", "firstName":"LL", "lastName":"Ops", "phone":""}'::json);
CALL public.upsert_user('{"id":0, "role":"admin", "email":"mmz@linuxlab.pro", "password":"Google2022", "firstName":"Myo", "lastName":"Min Zin", "phone":"959421076314"}'::json);
CALL public.upsert_user('{"id":0, "role":"user", "email":"mkzaw@linuxlab.pro", "password":"Google2022", "firstName":"Min", "lastName":"Khant Zaw", "phone":"959763250625"}'::json);
CALL public.upsert_user('{"id":0, "role":"guest", "email":"p.waiyamhein@linuxlab.pro", "password":"Google2022", "firstName":"Phone", "lastName":"Waiyam Hein", "phone":""}'::json);
